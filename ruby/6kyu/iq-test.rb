...
# that among the given numbers finds one that is different in evenness, and return a 
...
def iq_test(numbers)
  ...
  odds = []
  num_array = []
  numbers.split.each { |number| num_array << number.to_i }
  num_array.each_with_index { |num,index| num.even? ? evens << index + 1 : odds << index + 1}
