...
# For example if you are passed the list [4, 6] the equivalent 
# mathematical expression would be 4! + 6! which would equal 744.

# Good Luck!

# Note: Assume that all values in the list are positive integer 
# values > 0 and each value in the list is unique.

def sum_factorial(lst)
  lst.map { |factor|
    ((1..factor).to_a).inject(:*)
  }.inject(0,:+)
end
