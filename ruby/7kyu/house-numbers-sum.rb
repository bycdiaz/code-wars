...
# turns to change streets, so the numbers don't appear to him in any 
# particular order.
...
# For the given sequence of houses determine the sum that the boy will 
# get. It is guaranteed that there will always be at least one 0 house 
# on the path.
...
def house_numbers_sum(input_array)
  if input_array[0] == 0
    ...
    input_array[0...input_array.index(0)].reduce(:+)
  end
end

# refactored
def house_numbers_sum(input_array)
  input_array[0] == 0 ? input_array[0] : input_array[0...input_array.index(0)].reduce(:+)
end
