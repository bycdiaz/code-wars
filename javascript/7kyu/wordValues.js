...
// the strings as explained above multiplied by the position of that string in the
// list. For our purpose, position begins with 1.

// nameValue ["abc","abc abc"] should return [6,24] because of [ 6 * 1, 12 * 2 ]. Note
// how spaces are ignored.
...
const characterValue = (character) => [..."abcdefghijklmnopqrstuvwxyz"].indexOf(character) + 1;

function wordValue(strings) {
  const wordValues = [];
  ...
  strings.forEach((word, index) => {
    ...
    for (character of word) wordValue += characterValue(character);
    wordValues.push(wordValue * wordPosition);
  })
