...
function divisibleByLast(n) {
  ...
  n.toString().split('').forEach((current, index, array) => {
    const priorNumber = Number(array[index - 1]);
    const currentNumber = Number(current);
    const currentDivByPrior = currentNumber % priorNumber === 0;

    currentDivByPrior ? divisibilityChecks.push(true) : divisibilityChecks.push(false);
  });

  return divisibilityChecks;
}
