...
const sumAverage = (input) => {
  let arrAverages = [];
  const averages = (acc, cur) => acc + cur;
  input.forEach(array => {
    let arrSum = 0;
    array.forEach(num => { arrSum += num })
    arrAverages.push(arrSum / array.length);
  });
  return Math.floor(arrAverages.reduce(averages));
}
