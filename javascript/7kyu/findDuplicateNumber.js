...
function countElements(array) {
  return array.reduce((count, number) => {
    number in count ? count[number]++ : count[number] = 1;
    return count;
  }, {})
}

function findDup(numberArray) {
  const numberCount = countElements(numberArray);

  for (number of numberArray) {
    if (numberCount[number] > 1) return Number(number);
  }
}
