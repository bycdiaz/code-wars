...
function encode(string) {
  ...
  string.split('').forEach(char => {
    const vowelFound = vowels.includes(char);
    const encodedChar = vowels.indexOf(char) + 1;
    vowelFound ? encodedString.push(encodedChar) : encodedString.push(char);
  })
...
function decode(string) {
  const decodedString = [];
  string.split('').forEach(char => {
    const charANum = /[0-9]/.test(char);
    charANum ? decodedString.push(vowels[char - 1]) : decodedString.push(char);
  });
