...
function twoSum(numbers, target) {
  const potentialSecondNumbers = new Map();
  let numberIndices = -1;

  numbers.forEach((currentNumber, index) => {
    const currentDifference = target - currentNumber;

    if (potentialSecondNumbers.has(currentDifference)) {
      numberIndices = [potentialSecondNumbers.get(currentDifference), index];
    } else {
      potentialSecondNumbers.set(currentNumber, index);
    }
  })
